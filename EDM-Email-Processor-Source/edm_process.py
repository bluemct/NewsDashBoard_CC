...
def convert_xlsx_to_csv(xlsx_path):
    """Convert .xlsx to CSV using xlsx_to_csv.py (same directory, or skills path)."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    xlsx_skill = os.path.join(script_dir, "xlsx_to_csv.py")
    if not os.path.isfile(xlsx_skill):
        # Fallback: try original skills path
...
def generate_formal_test_csv(xlsx_path):
    ...
    # Load test emails from config.json (try script dir first, then project root)
...
def convert_msg_to_html(msg_path, output_html):
    ...
    # Load token mapping from Tokenmapping.json (try script dir first, then project root)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    mapping_path = os.path.join(script_dir, "Tokenmapping.json")
