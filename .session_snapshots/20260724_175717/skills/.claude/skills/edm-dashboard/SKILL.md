...
- 仅展示**至少有一封** subject 含 `[EDM test and distribution]` 的邮件对话
- **两步过滤**：先按 EDM 前缀找到 conversation_id，再取该 conversation **全部邮件**（含 `Request #xxx` 工单自动回复、`EDM test_` 前缀等非 EDM 前缀邮件），确保步骤号完整
...
| [4.5] | 按 SN 合并多 conversation | 同一 SN 有多个 conversation_id 时，取邮件数最多的为主，其余合并 |
