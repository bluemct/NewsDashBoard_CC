...
def main():
    ...
    parser.add_argument("--pid-file", default=None, help="Write PID to file for one-instance check")
    ...
    if _pid_file:
        try:
            Path(_pid_file).write_text(str(_os.getpid()))
        except Exception:
            pass

    # Log start
    _LOG = Path(__file__).parent / ".usage_monitor_debug.log"
    with open(_LOG, "a", encoding="utf-8") as _f:
        _f.write(f"[usage_monitor] Started pid={_os.getpid()}\n")
