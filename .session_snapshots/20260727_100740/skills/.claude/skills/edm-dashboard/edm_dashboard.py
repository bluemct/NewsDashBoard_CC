...
def build_conversations(raw):
    handlers = _handlers
    # First pass: find conversation_ids that have at least one [EDM test and distribution] email
    edm_convs = set()
    for r in raw:
        if "[EDM test and distribution]" in r.get("subject", ""):
            edm_convs.add(r["conversation_id"])
    # Second pass: collect ALL emails for those conversations (incl. Request #xxx auto-replies)
    convs = {}
    for r in raw:
        if r["conversation_id"] in edm_convs:
    ...
    for cid, emails in convs.items():
        ...
        # Use the first [EDM test and distribution] email as the subject/date source
        edm_emails = [e for e in emails if "[EDM test and distribution]" in e.get("subject", "")]
        if not edm_emails:
            continue
        subject = edm_emails[0]["subject"]
        first_date = edm_emails[0].get("date", "")[:10]
...
def compute_weekly_stats(raw):
    ...
    try:
        ...
        # Group into conversations — find EDM conv IDs first, then include ALL emails
        edm_convs = set()
        for r in raw:
            if "[EDM test and distribution]" in r.get("subject", ""):
                edm_convs.add(r["conversation_id"])
